...
void run_on_gpu(_t* offset, _t offset_len, _t* detail, size_t detail_len, _t N, int, int, _t*);

void get_data(const char* filename, _t** offset, _t* offset_len, _t**detail, size_t* detail_len, _t* N, _t** cont_nodes);
...
{	
	...
	if(taskId == 0)//master
	{
		_t* offset, *detail, *cont_nodes;
		_t offset_len, N;
		size_t detail_len;				
		
		get_data("graph.txt", &offset, &offset_len, &detail, &detail_len, &N, &cont_nodes);
		...
		if(numTasks > 1)
		{
			...
			for(slave = 1; slave < numTasks; ++slave)
			{
				...
				MPI_Send(cont_nodes, N, MPI_SHORT, slave, 1, MPI_COMM_WORLD);
			}
			gettimeofday(&stop, 0);
		...
		gettimeofday(&start, 0);
		run_on_gpu(offset, offset_len, detail, detail_len, N, taskId, numTasks, cont_nodes);
		gettimeofday(&stop, 0);
	...
	else
	{
		...
		MPI_Recv(cont_nodes, N, MPI_SHORT, 0, 1, MPI_COMM_WORLD, &status);
